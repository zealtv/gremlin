...
**Outcome.** Prove the memory integration works end to end: `/new`
reviews a completed session, `/discard` does not, the memory model
...
- Verify `/new` archives the transcript and queues a memory-review item.
...
- Verify `/discard` archives without queueing memory review.
- Verify `/new` and `/discard` session commands behave correctly.
...
9. Run `/new` or `/discard` to clear the active transcript.
10. Ask a question that depends on the preference. The answer should use the
    promoted context finding.
11. Repeat with a temporary preference and `/discard`; confirm no
