...
- Added `/new-session` to archive the current transcript, start fresh, and queue
...
- Added `/discard-session` to archive and start fresh without memory review.
- Made `/new` delegate to `/new-session`.
- Added `/discard` as an alias for `/discard-session`.
- Updated command docs and user-facing README text.

Checks run in `/private/tmp/gremlin-s51-test`:

```sh
bash -n .gremlin/commands/new-session.sh .gremlin/commands/discard-session.sh .gremlin/commands/discard.sh .gremlin/commands/new.sh .gremlin/bin/archive.sh
/private/tmp/gremlin-s51-test/.gremlin/commands/new-session.sh
...
/private/tmp/gremlin-s51-test/.gremlin/commands/discard-session.sh
...
`/new-session` and `/new` queued memory-review items. `/discard-session` and
`/discard` did not increase the memory-review item count. `/help` listed the
long forms and aliases.
