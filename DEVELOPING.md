...
- `cd ~/Desktop/nestlings\ family/gremlin`
- Pick a host directory for your personal gremlin somewhere **outside** the repo: `mkdir -p ~/Desktop/my-gremlin-host`
- Once `s05a-init-script` is tied, run: `./init.sh ~/Desktop/my-gremlin-host` → produces `~/Desktop/my-gremlin-host/.gremlin/`
- Until `init.sh` exists: `cp -r .gremlin ~/Desktop/my-gremlin-host/.gremlin`
- Personalise *your* copy only: edit `~/Desktop/my-gremlin-host/.gremlin/gremlin.md`, drop facts into `context/*.md`. Never edit identity/context inside the repo.
...
- **Sync into your personal copy** to test:
  ...
  rsync -a \
    ...
    --exclude='context/' \
    .gremlin/ ~/Desktop/my-gremlin-host/.gremlin/
  ```
...
- **Run from the personal copy:** `cd ~/Desktop/my-gremlin-host && ./.gremlin/run.sh` (or invoke the specific script the stitch is testing).
- Test the stitch's *Verify* clause against the personal copy. Iterate: edit in repo → rsync → re-test.
- When green: `cd ~/Desktop/nestlings\ family/gremlin && ./.loom/loom.sh tie <stitch-id>`
...
- **`git status` is the canary.** If anything under the paths above changes inside the repo, you ran something against the wrong copy. `git checkout -- <path>` to discard.
- **Personalisation lives only outside the repo.** Your real `gremlin.md`, your real `context/user.md`, your real schedule items, your real transcripts — all in `~/Desktop/my-gremlin-host/.gremlin/`, never the repo.
...
```bash
cd ~/Desktop/my-gremlin-host && ./.gremlin/bin/archive.sh
```

Until then, by hand:

```bash
cd ~/Desktop/my-gremlin-host/.gremlin
touch .paused
...
- [ ] `git status` is clean from the repo root
