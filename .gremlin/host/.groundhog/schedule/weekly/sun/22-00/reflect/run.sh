...
# of its schedule depth, so walk up to the directory that holds .nest. That is
# the HOST root — the nest, the schedule and this job all belong to the
# repository; only the gremlin's own files are under .gremlin/.
HOST_DIR="$(d="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"; \
  while [ "$d" != / ] && [ ! -d "$d/.nest" ]; do d="$(dirname "$d")"; done; \
  printf %s "$d")"
GREMLIN_DIR="$HOST_DIR/.gremlin"
[ -d "$HOST_DIR/.nest" ] || { echo "reflect: could not locate the host root" >&2; exit 1; }
[ -d "$GREMLIN_DIR" ] || { echo "reflect: no .gremlin at $HOST_DIR" >&2; exit 1; }

# The cursor lives in the schedule SOURCE (in the host's .groundhog, which
# /update cannot reach at all) — never this materialised copy, which is
# discarded after the tend.
STATE="$HOST_DIR/.groundhog/schedule/weekly/sun/22-00/reflect/cursor"
