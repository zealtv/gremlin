...
# case the vendored copy of one of these never wins — where the host has its
# own, the vendored one is parked for review. Where the host has NO copy the
# vendored one moves in: a primitive missing its README or its script is an
# incomplete install, and parking the only copy is how you get one. (Learned
# the hard way on HQ: the host .lore had no README.md, so the map generated by
# index-primitives.sh silently omitted lore entirely.) Everything else is
# data: it merges, and a genuine same-path conflict refuses the whole run.
host_owned() {
  case "$1" in
...
plan_merge() {  # plan_merge <primitive> <srcdir> <dstdir> <relprefix>
  ...
  for entry in "${kids[@]}"; do
    ...
    s="$srcdir/$name"
    d="$dstdir/$name"
    if is_host_owned "$p" "$rel" && { [ -e "$d" ] || [ -L "$d" ]; }; then
